...
function frontPageInitEvents($container) {
  ...
  scrollAnimator = new ExtraScrollAnimator({
    ...
    tween: new TimelineMax().staggerFrom(
      $container.find('.events-wrapper .event-link'),
      1,
      {
        y: 200,
        ease: Quad.easeOut,
...
function frontPageInitSelection($container) {
  ...
  function updateItems() {
    items.forEach(function(item, index) {
      if (
        -Math.floor(item.offset + itemWidth) >= Math.ceil(tmpSliderX) &&
        !item.isLeft
      ) {
        item.isLeft = !0;
  ...
  function windowResizeHandler() {
    ...
    draggerSnap = [];
    var sliderRatio =
      (draggerWidth - handleWidth) / (sliderInnerWidth - sliderWidth);
    $sliderInner.children('.movie').each(function(index, item) {
      ...
      draggerSnap.push(
        Math.min(draggerWidth - handleWidth - 1, itemLeft * sliderRatio),
      );
    });
...
function transitionIn($container) {
  ...
  timeline.staggerFrom(
    $container.find('.header .title .letter'),
    0.6,
    {
      cycle: {
...
function extraBlockUI(element, event) {
  ...
  $('html')
    .addClass('extra-transition-freeze')
    .on('scroll touchmove mousewheel', blockScroll);
}

function extraReleaseUI(element, event) {
  app.isTransitioning = !1;
  $('html')
    .removeClass('extra-transition-freeze')
    .off('scroll touchmove mousewheel', blockScroll);
}
